...
def listener(messages):
    ...
    for m in messages:
        try:
            file_id, file_info = get_name_from_message(m)
            if file_id not in files_downloaded:
...
def main():
    ...
    def callbacks(m):
        ...
        if m.data == 'upload':
            ...
            file_id, file_info = get_name_from_message(origin_message)

            # Check if the file was downloaded previously
            if file_id not in files_downloaded:
                # add new file to the record
                files_downloaded.append(file_id)
                aux = open(path_file_downloaded, 'a')
                aux.write('{} {}\n'.format(datetime.date.today().strftime('%Y-%m-%d'), file_id))
                aux.close()
